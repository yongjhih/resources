...
fastZombies = { 264,277,287 }
slowZombies = { 13,22,56 }
...
function Zomb_chase (ped, Zx, Zy, Zz )
	if isElement(ped) then
	local zskin = getElementModel ( ped )
				...
				if (getElementData ( ped, "target" ) == nil) then
					...
					if giveup == 1 then
						...
						if action == 1 then
							...
							chaseanim = "run_old"
							for k, skin in pairs( slowZombies ) do
								if skin == zskin then
									chaseanim = "WALK_drunk"
								end
							end
							for k, skin in pairs( fastZombies ) do
								if skin == zskin then
									chaseanim = "Run_Wuzi"
								end
							end
		...
		if (getElementData (source, "zombie") == true) and (getElementType ( source ) == "ped") then
			if ((getElementType ( attacker ) == "player") or (getElementType ( attacker ) == "vehicle")) then
				...
				if oldZcount ~= false then
					...
					triggerClientEvent ( "onZombieWasted", getRootElement(), ammo, owner, weapon, bodypart )
				else
